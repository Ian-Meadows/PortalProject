...
/*all of this is taken from the CSCIx229 library*/
...
namespace TextureHandler{
    ...
    unsigned int LoadBMPTexture(const char* file){
        ...
        if (dx<1 || dx>(unsigned)max) Fatal("%s image width %d out of range 1-%d\n",file,dx,max);
        if (dy<1 || dy>(unsigned)max) Fatal("%s image height %d out of range 1-%d\n",file,dy,max);
    ...
    unsigned int LoadBMPTexture32(const char* file){
        ...
        glBindTexture(GL_TEXTURE_2D,texture);
        //  Copy image
        glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,dx,dy,0,GL_RGBA, GL_UNSIGNED_BYTE, image);
        if (glGetError()) Fatal("Error in glTexImage2D %s %dx%d\n",file,dx,dy);
        //  Scale linearly when image size doesn't match
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
