...
namespace TextureHandler
{
    ...
    unsigned int LoadTexture(std::string file, bool hasAlpha)
    {
        ...
        if (hasAlpha)
        {
            result = TextureHandler::LoadBMPTexture32(file.c_str());
        }
        else
        {
            result = TextureHandler::LoadBMPTexture(file.c_str());
        }
        textures.emplace(file,result);
        return result;
    }
    ...
    unsigned int LoadBMPTexture(const char *file)
    {
        ...
        if (dx < 1 || dx > (unsigned)max)
            Fatal("%s image width %d out of range 1-%d\n", file, dx, max);
        if (dy < 1 || dy > (unsigned)max)
